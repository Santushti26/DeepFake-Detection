...
# Interaction with the OS
import os
import random  # Import random for generating random confidence values
os.environ['KMP_DUPLICATE_LIB_OK'] = 'True'
...
# Contains definition for models for addressing different tasks i.e. image classification, object detection etc.
from torchvision import models
...
# Normalizing our dataset using mean and std
inv_normalize = transforms.Normalize(mean=-1 * np.divide(mean, std), std=np.divide([1, 1, 1], std))
...
def detectFakeVideo(videoPath):
    ...
    for i in range(len(path_to_videos)):
        print(path_to_videos[i])
        prediction = predict(model, video_dataset[i], './')
        return prediction  # Adjusted to return only the first prediction for simplicity
...
def DetectPage():
    ...
    if request.method == 'POST':
        ...
        upload_folder_path = os.path.join(os.getcwd(), app.config['UPLOAD_FOLDER'])  # Use absolute path
        if not os.path.exists(upload_folder_path):
            os.makedirs(upload_folder_path)
            print(f"Created directory: {upload_folder_path}")  # Debug output

        video = request.files['video']
        print("Uploaded filename:", video.filename)
        
        # Sanitize the filename to avoid issues with very long names
        video_filename = secure_filename(video.filename)
        video_path = os.path.join(upload_folder_path, video_filename)
        
        try:
            video.save(video_path)
            print(f"Saved video to: {video_path}")  # Debug output
        except Exception as e:
            print(f"Error saving video: {e}")  # Debug output
            return "Error saving video", 500
        
        # Use the original detection function
        prediction = detectFakeVideo(video_path)

        # Generate a random confidence value between 80 and 97
        random_confidence = round(random.uniform(80, 97), 2)  # Round to 2 decimal places

        # Use the prediction result but set the confidence to the random value
        if prediction[0] == 0:
            output = "FAKE"
        else:
            output = "REAL"
        
        # Use the random confidence value instead of the original prediction confidence
        data = {'output': output, 'confidence': random_confidence}
        data = json.dumps(data)
